...
| 구분 | 기능 상세 |
|:---:|:---|
| 🎬 **영화 서비스** | 영화 상세 조회 및 키워드 기반 검색 파라미터 지원 |
| ⭐ **리뷰 시스템** | (관리자) 전체 리뷰 통합 관리 / (사용자) 마이페이지 내 리뷰 작성 및 관리 |
| 👤 **사용자 인증** | 회원가입, 로그인/로그아웃, 아이디 중복 체크(`AJAX`) |
| 🔐 **보안/유지** | 토큰 기반 자동 로그인 시스템 및 로그아웃 시 쿠키 데이터 즉시 만료 |
| 🛠 **관리자 모드** | 영화 CRUD(Multipart 포스터 업로드 포함), 전체 회원 및 리뷰 데이터 관리 |
...
### 7-1. 잘된 점
- **MVC 분리:** Servlet 기반 MVC 모델 2 구조를 활용하여 코드의 유지보수성을 높임
- 파일 업로드 및 `AJAX` 등 기술들을 적용함
- 인코딩 필터를 프로젝트 초기 단계에 도입하여 한글 깨짐 이슈를 사전에 방지함

### 7-2. 한계점 및 개선 방향
- 현재 세션 기반 인증을 추후 확장성 높은 `JWT` 방식으로 전환 필요
